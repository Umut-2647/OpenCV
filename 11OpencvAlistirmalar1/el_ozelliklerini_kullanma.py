...
def findMAxContour(contours):           #maks konturları bulmak için bir fonksiyon tanımlıyoruz
    ...
    for i in range(len(contours)):
        area_hand=cv2.contourArea(contours[i])   ##burda her değer için kontur alanı hesaplanıyor ve if bloğuyla en büyük olan max_area değişkenine aktarılıyor
...
while 1:
    ...
    if len(contours)>0:              ##hata alama ihtimalimiz oldugu için try expect blogu açıyoruz
        ...
        try:
            ...
            if ab_arasi_aci>70:
                cv2.rectangle(frame, (0, 0), (100, 100), (255, 0, 0), -1)  # (x1,y1) (x2,y2)
            else :
                pass
